...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { email, room_id, notes, code, language, candidate_name } = req.body as EmailRequest
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: candidate_name
          ? `Interview Recap — ${candidate_name}`
          : `Interview Recap — Session (${room_id})`,
        html: build_email_html({ notes, code, language, candidate_name, room_id }),
      }),
    })

    if (!response.ok) {
      const err_data = await response.json() as { message?: string }
      return res.status(500).json({ error: err_data.message || 'Failed to send email' })
    }
